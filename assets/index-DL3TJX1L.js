...
`,'"',m.BYTE_ORDER_MARK],m.WORKERS_SUPPORTED=!p&&!!d.Worker,m.NODE_STREAM_INPUT=1,m.LocalChunkSize=10485760,m.RemoteChunkSize=5242880,m.DefaultDelimiter=",",m.Parser=N,m.ParserHandle=Q,m.NetworkStreamer=_,m.FileStreamer=A,m.StringStreamer=k,m.ReadableStreamStreamer=w,d.jQuery&&((l=d.jQuery).fn.parse=function(S){var q=S.config||{},L=[];return this.each(function(le){if(!(l(this).prop("tagName").toUpperCase()==="INPUT"&&l(this).attr("type").toLowerCase()==="file"&&d.FileReader)||!this.files||this.files.length===0)return!0;for(var Me=0;Me<this.files.length;Me++)L.push({file:this.files[Me],inputElem:this,instanceConfig:l.extend({},q)})}),W(),this;function W(){if(L.length===0)F(S.complete)&&S.complete();else{var le,Me,qe,O,Y=L[0];if(F(S.before)){var ee=S.before(Y.file,Y.inputElem);if(typeof ee=="object"){if(ee.action==="abort")return le="AbortError",Me=Y.file,qe=Y.inputElem,O=ee.reason,void(F(S.error)&&S.error({name:le},Me,qe,O));if(ee.action==="skip")return void $();typeof ee.config=="object"&&(Y.instanceConfig=l.extend(Y.instanceConfig,ee.config))}else if(ee==="skip")return void $()}var ae=Y.instanceConfig.complete;Y.instanceConfig.complete=function(j){F(ae)&&ae(j,Y.file,Y.inputElem),$()},m.parse(Y.file,Y.instanceConfig)}}function $(){L.splice(0,1),W()}}),b&&(d.onmessage=function(S){S=S.data,m.WORKER_ID===void 0&&S&&(m.WORKER_ID=S.workerId),typeof S.input=="string"?d.postMessage({workerId:m.WORKER_ID,results:m.parse(S.input,S.config),finished:!0}):(d.File&&S.input instanceof File||S.input instanceof Object)&&(S=m.parse(S.input,S.config))&&d.postMessage({workerId:m.WORKER_ID,results:S,finished:!0})}),(_.prototype=Object.create(h.prototype)).constructor=_,(A.prototype=Object.create(h.prototype)).constructor=A,(k.prototype=Object.create(k.prototype)).constructor=k,(w.prototype=Object.create(h.prototype)).constructor=w,m})}(Yo)),Yo.exports}var eb=Wv();const ab=O2(eb),rb=`03000000;Productos de la agricultura, ganadería, pesca, silvicultura y productos afines\r
